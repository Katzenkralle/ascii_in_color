...
class AsciiInColor(ImageReader):
    ...
    def print_ascii_img(self, colorize_part: str = "False"):
        #Prints the ascii art
        for row in self.ascii_img:
            if not self.is_colorized:
                #If the image is not colorized, each row is printed as a string 
                print("".join(row))
            else:
                for char, rgba in row:
                        #If the image is colorized, each char is printed with the color
                        if rgba == (0, 0, 0, 0):
                            #If the char is whitespace, the background is set to trabsparent
                            print(bg.rs + fg.rs + " ", end="")
                            continue
                        
                        #each char is printed with its color, applied according to colorize_part
                        ...
                        if "b" in colorize_part and "f" in colorize_part:
                            print(bg.char_color + fg.char_color + char, end="")
                        elif "b" in colorize_part:
                            print(bg.char_color + char, end="")
                        else:
                            print(fg.char_color + char, end="")
                #Creates newline (with a transparent char)
                print(bg.rs + fg.rs + "\n", end="")
